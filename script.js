...
document.querySelectorAll(".project-browser").forEach(browser=>{
  const cards=[...browser.querySelectorAll(".project-card")], per=Number(browser.dataset.perPage)||4, pager=browser.querySelector(".pager");
  const pages=Math.ceil(cards.length/per); let current=1;
  function render(page){
    current=page;
    cards.forEach((card,i)=>card.hidden=!(i>=(page-1)*per&&i<page*per));
    if(!pager)return;
    pager.innerHTML="";
    if(pages<=1)return;
    for(let p=1;p<=pages;p++){
      const b=document.createElement("button"); b.type="button"; b.textContent=p; b.className=p===current?"active":"";
      b.setAttribute("aria-label",`Show project page ${p}`);
      b.addEventListener("click",()=>{render(p);browser.scrollIntoView({behavior:"smooth",block:"start"});});
      pager.appendChild(b);
    }
  }
  render(1);
